...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    if($fileName !== ""){  // ファイルが選択されたら
        ...
        foreach ($data as $key => $row) {
            ...
            if(count($row) != 2){ //カラム数が不一致の場合フォーマット違いエラー
                ...
                if($row[0] == "" || $row[1] == ""){
                    $errs['errors'] = "メールアドレスか名前に空白セルがあります。";
                }
            }
        }
...
<head>
    <meta charset="utf-8">
    <title>CSV教員一括登録</title>
    <link href="CSV_upload_teacher.css" rel="stylesheet" />
</head>
<body>
...
<main class="main-content">  
    ...
    <div class="teachUpload-container">
        <h1>CSV教員一括登録</h1>

    <div class="btn-container">
        <p id="u">ユーザー種別</p>
        <input type="button" onclick="location.href=''" id="teacher_btn" value="教員">
        <input type="button" onclick="location.href='./CSV_upload_student.php'" id="stu_btn" value="学生">
        <br>
    </div>
        ...
        <script>
            ...
            <p><a href="TeacherCSV Template.csv" download="TeacherCSV Template" id="template"> 教員CSVアップロード用テンプレート</a></p>

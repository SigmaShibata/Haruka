<?php
    ...
    if (!isset($_GET['kadai_id']) || empty($_GET['kadai_id'])) {
        header('Location: ../課題管理リスト/kadaiList.php');
        exit;
    }

    // 課題管理リストから課題IDを取得して該当する課題のデータを取得
    $kadai_id = (int)$_GET['kadai_id'];
    $kadai = $kadaiDAO->get_kadai_by_kadai_id($kadai_id);

    if (!$kadai) {
        echo "指定された課題が存在しません。課題管理リストから選択ください。";
        exit;
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
        ...
        if(empty($errs)){
            ...
            // 更新完了メッセージを表示して、リダイレクトする（課題管理リストのページURL）
            echo "<script>alert('課題が更新されました。'); window.location.href = '../課題管理リスト/kadaiList.php';</script>";
            exit;
        }
    }

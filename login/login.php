<?php
    ...
    if(!empty($_SESSION["member"])){
      ...
      if($member->is_initial_login === true){
        header('Location: ../change_password/passhenkou.php'); //初期パスワード変更に
        exit;
      }

      elseif($member->user_type == 1){
        header('Location: ../stamp_sheet/stamp_sheet.php'); //学生ページに
        exit;
      }

      else{
        header('Location: ../check_list/kadaiChecklist.php'); //先生ページに
        exit;
      }    
    }
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST'){
        ...
        if(empty($errs)){
            ...
            if($member !== false){
                ...
                if($member->is_initial_login === true){
                  header('Location: ../change_password/passhenkou.php'); //初期パスワード変更ページに
                  exit;
                }
